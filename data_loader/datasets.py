...
class datasets(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            sampled_frame_idx = sampled_frame_idx.clip(min = self.idx_frame_flat[index], max = len(input_file_path) - 1)
        else:
        ...
        for frame_idx in range(len(sampled_frame_idx)):
            ...
            input_img = read_frame(input_file_path[sampled_idx], None, rotate_val, flip_val, gauss)
            gt_img = read_frame(gt_file_path[sampled_idx], None, rotate_val, flip_val, gauss)
