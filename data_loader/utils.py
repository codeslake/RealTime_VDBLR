...
def read_frame(path, norm_val = None, rotate_val = None, flip_val = None, gauss = None):
    if norm_val == (2**16-1):
    ...
    if gauss is not None:
        ...
        frame = np.clip(frame + gauss, 0, 1.0)

    # frame = (frame * 2.) - 1.

    return np.expand_dims(frame, axis = 0)

def augment(image, rotate_val = None, flip_val = None, gauss = None):
    frame = image / 255.
    frame = frame[...,::-1]
