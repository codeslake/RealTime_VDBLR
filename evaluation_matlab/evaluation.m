function [psnr_mean, ssim_mean] = evaluation(test_model, folder_result, test_set, test_offset)
    ...
    elseif strcmp(test_set, 'REDS')
        folder_gt = fullfile(test_offset, 'valid_REDS', 'gt');
    end
...
%     cropsize = 16;
    for i = 1:length(video_paths)
        ...
        elseif strcmp(test_set, 'REDS')
            frame_path_GT = dir(fullfile(folder_gt, video_paths(i).name,'*.png'));
        end

        frame_path_result = dir(fullfile(folder_result, video_paths(i).name, '*.png'));
        if length(frame_path_result) == 0
            frame_path_result = dir(fullfile(folder_result, video_paths(i).name, '*.jpg'));
        end
...
%             input = input(1+cropsize:end-cropsize,1+cropsize:end-cropsize,:);    
            ...
            psnr_ = psnr(GT,input);
            psnr_list_total_frame = [psnr_list_total_frame, psnr_];
            ssim_ = ssim(GT,input);
            ssim_list_total_frame = [ssim_list_total_frame, ssim_];
...
%             fprintf('[%s on %s] [%d/%d] [%d/%d] [gt_name: %s / input_name: %s] PSNR: %0.4f, SSIM: %0.4f\n', test_model, test_set, i, length(video_paths), j, num, frame_path_GT(GT_idx).name, frame_path_result(j).name, psnr_, ssim_);
    ...
    psnr_mean = mean(psnr_list_total_frame);
    ssim_mean = mean(ssim_list_total_frame);
%     fprintf('[%s on %s]\n', test_model, test_set);
%     fprintf('PSNR: %f\n', psnr_mean);
%     fprintf('SSIM: %f\n', ssim_mean);
