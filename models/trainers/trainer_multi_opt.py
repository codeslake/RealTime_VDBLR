...
class Model(baseModel):
    def __init__(self, config):
        ...
        if self.is_train and self.config.resume is None or self.is_train and os.path.exists('./models/archs/{}.py'.format(config.network)):
            copyfile('./models/archs/{}.py'.format(config.network), os.path.join(self.config.LOG_DIR.offset, '{}.py'.format(config.network)))
            copyfile('./models/trainers/{}.py'.format(config.trainer), os.path.join(self.config.LOG_DIR.offset, '{}.py'.format(config.trainer)))
    ...
    def _set_optim(self, lr = None):
        ...
        ## for the motion loss (distillation loss)
        optimizer = torch.optim.Adam([
