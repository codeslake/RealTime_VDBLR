...
def get_config(project = '', mode = '', config = '', data = '', LRS = '', batch_size = 8):
    ...
    config.skip_corr_index = [1] # 0(m)1(skip)2(m)3
    ...
    config.lr_min = 1e-4 * 0.25 * 0.25
    if config.LRS == 'LD':
        # lr_decay
        config.decay_period = [1000000, 1250000]
        #config.decay_period = [750000, 950000]
        config.decay_rate = 0.25
        config.warmup_itr = -1
    elif config.LRS == 'CA':
        # Cosine Anealing
        config.warmup_itr = -1
        config.T_period = [0, 1500200, 1500000]
